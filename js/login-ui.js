/**
 * Login UI + navegação shell (funciona mesmo antes do app.js carregar)
 */
(function () {
  'use strict';
  ...
  /** Troca painel interno (menu lateral) */
  ...
  async function loadPerfilData(supabase) {
    ...
    if (!perfilResult.data) {
      throw new Error('Sem perfil no banco. Execute sql/seed-dados-demo.sql.');
    }
  ...
  async function activateAppAfterLogin() {
    if (typeof window.__CPE_routeByProfile === 'function') {
      window.__CPE_routeByProfile();
      return;
    }

    var appReady = await waitForApp(8000);
    if (appReady && typeof window.__CPE_routeByProfile === 'function') {
      window.__CPE_routeByProfile();
    } else {
      window.dispatchEvent(new CustomEvent('cpe:logged-in'));
      showLoginMessage(
        'Painel aberto. Se os dados não carregarem, recarregue a página (F5).',
        'info'
      );
    }
  }
  ...
  async function handleLogoutClick() {
    try {
      ...
      if (window.__CPE_SUPABASE__) {
        await window.__CPE_SUPABASE__.auth.signOut();
      }
      window.__CPE_SESSION__ = null;
      showSection('section-login');
      showLoginMessage('Você saiu do sistema.', 'info');
      $('form-login')?.reset();
    } catch (err) {
      showLoginMessage('Erro ao sair: ' + err.message, 'error');
    }
  }

  /** Delegação: menu lateral e botões Sair */
  ...
  function initLoginUI() {
    ...
    try {
      validateConfig();
      showLoginMessage('Sistema pronto. master@demo.com / 123456', 'info');
    } catch (err) {
